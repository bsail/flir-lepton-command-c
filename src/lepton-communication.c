...
static inline void sendCommand_common(struct lepton_communication * this,uint16_t cmdCode,uint16_t * value,int dataLength,uint8_t isRaw)
{
  if(this!=0) {
...
static inline void receiveCommand_common(struct lepton_communication * this,uint16_t cmdCode,uint16_t * value,int dataLength)
{
  if(this!=0) {
    if((value!=0)&&(dataLength!=0)) {
      if (waitCommandBegin(this,LEPFLIR_GEN_CMD_TIMEOUT)) {
