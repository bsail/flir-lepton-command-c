...
static inline void get_common(struct lepton_driver * driver, enum transmission_mode mode, uint16_t cmdID, uint16_t cmdType, uint16_t * data, uint16_t length)
{
  switch(mode){
    case u16:receiveCommand_u16(&(driver->communication),cmdCode(cmdID,cmdType),data);
      break;
    case u32:receiveCommand_u32(&(driver->communication),cmdCode(cmdID,cmdType),(uint32_t *)data);
      break;
    case array:receiveCommand_array(&(driver->communication),cmdCode(cmdID,cmdType),(uint16_t *)data,length);
      break;
  }
...
LEP_SYS_CAM_STATUS_STATES getCameraStatus(struct lepton_driver * driver)
{
  ...
  if(driver!=0) {
    ...
    get_common(driver,array,LEP_CID_SYS_CAM_STATUS,LEP_I2C_COMMAND_TYPE_GET,(uint16_t *)&camStatus,sizeof(LEP_SYS_CAM_STATUS) / 2);
