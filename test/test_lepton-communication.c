...
void test_receiveCommand_u32_should_work(void)
{
  ...
  readDataRegister_ReturnMemThruPtr_readWords((uint16_t *) & value,
                                              sizeof(value));
...
void test_cmdCode_GET_random_address(void)
{
  for (uint16_t i = 0; i < 20; ++i) {
    ...
    TEST_ASSERT_EQUAL_HEX16(expected,
                            cmdCode(random, LEP_I2C_COMMAND_TYPE_GET));
  }
}
...
void test_cmdCode_SET_random_address(void)
{
  for (uint16_t i = 0; i < 20; ++i) {
    ...
    TEST_ASSERT_EQUAL_HEX16(expected,
                            cmdCode(random, LEP_I2C_COMMAND_TYPE_SET));
  }
}
...
void test_cmdCode_RUN_random_address(void)
{
  for (uint16_t i = 0; i < 20; ++i) {
    ...
    TEST_ASSERT_EQUAL_HEX16(expected,
                            cmdCode(random, LEP_I2C_COMMAND_TYPE_RUN));
  }
}
